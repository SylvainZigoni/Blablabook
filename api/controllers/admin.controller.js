...
const adminController = {
	...
	async updateCategory(req, res) {
		...
		if (
			...
			return res
				.status(StatusCodes.BAD_REQUEST)
				.json({ message: "Les noms de catégorie sont requis." });
		}
		...
		try {
			...
			// Vérifier que ce n'est pas la même catégorie (parseInt permet de transformer un string en nombre)
	...
	async addAuthor(req, res) {
		try {
			...
			if (!name || typeof name !== "string" || name.trim() === "") {
				return res
					.status(StatusCodes.BAD_REQUEST)
					.json({ message: 'Le champ "name" est requis.' });
			}
	...
	async deleteAuthor(req, res) {
		try {
			...
			// Lors d'un destroy sequelize renvoie 1 si ok et 0 si non ok
			if (deletedAuthor === 0) {
				return res
					.status(StatusCodes.NOT_FOUND)
					.json({ message: "Auteur non trouvé." });
			}
	...
	// Inutilisée pour le moment, mais sera utile lors de la possibilité pour l'user de modifier ses infos
	...
	async updateBook(req, res) {
		...
		const updateBookSchema = Joi.object({
			...
			date_parution: Joi.date().max("now").messages({
				"date.max":
					"La date de parution ne peut pas être dans le futur.",
			}),
		...
		try {
			...
			const { error, value } = updateBookSchema.validate(req.body, {
				abortEarly: false, // Permet de continuer meme s'il rencontre une erreur, afin de renvoyer toutes les erreurs rencontrées à la fin
				stripUnknown: true, // Permet de ne pas prendre en compte les champs non définis dans le schéma (pour proteger les données)
			});
			...
			// Sequelize génère ses noms d'erreur en fonction des erreurs. Selon le nom de l'erreur on traitera l'objet erreur renvoyé de facon differente
		} catch (error) {
	...
	async createBook(req, res) {
		...
		const createBookSchema = Joi.object({
			...
			summary: Joi.string().trim().allow("", null),
			date_parution: Joi.date().max("now").allow(null).messages({
				"date.max":
					"La date de parution ne peut pas être dans le futur.",
			}),
			...
			authorId: Joi.number().integer().positive().allow(null).messages({
		...
		try {
			...
			// On créé d'abord le livre et ensuite on ajoutera les informations qui sont dans les autres tables (author et categories)
